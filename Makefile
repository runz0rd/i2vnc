...
config = sudo mkdir -p /var/run/i2vnc && sudo cp config.yaml /var/run/i2vnc/config.yaml
...
clean = sudo rm -rf $(srcdir) /usr/share/X11/xorg.conf.d/xorg.conf /lib/systemd/system/i2vnc.service /usr/local/bin/i2vnc /var/run/i2vnc/
